...
export function ActionComposer({ branches, qualitiesCarried, onChange }: Props) {
  ...
  return (
    <div className="card action-card">
      <h3>One present action</h3>
      <p className="hint">
        One coherent movement that carries what returned. Name it and describe it — the rest is
        optional.
      </p>
      <div className="field">
        <label htmlFor="act-title">Name it</label>
        <input
          id="act-title"
          value={title}
          onChange={(e) => { setTitle(e.target.value); emit({ title: e.target.value }); }}
          placeholder="e.g. An evening that carries everything"
        />
      </div>
      <div className="field">
        <label htmlFor="act-instr">The movement itself</label>
        <textarea
          id="act-instr"
          value={instruction}
          onChange={(e) => { setInstruction(e.target.value); emit({ instruction: e.target.value }); }}
          placeholder="e.g. Eat a proper meal, a twenty-minute workout, then define tomorrow's one meaningful work action."
        />
      </div>

      <details className="optional-details">
        <summary>Shape it further (optional)</summary>
        <div className="field" style={{ display: "flex", gap: "0.75rem", flexWrap: "wrap" }}>
          <div style={{ flex: 1, minWidth: 140 }}>
            <label htmlFor="act-duration">About how long? (minutes)</label>
            <input
              id="act-duration"
              type="number"
              min={5}
        ...
        <div className="field">
          <label htmlFor="act-min">The smallest version that still counts</label>
          <input
            id="act-min"
            value={minimum}
            onChange={(e) => { setMinimum(e.target.value); emit({ minimum: e.target.value }); }}
            placeholder="e.g. Ten minutes of movement and one written sentence"
          />
        </div>
        <div className="field">
          <label htmlFor="act-done">You will know it is complete when…</label>
          <input
            id="act-done"
            value={completion}
        ...
        {branches.length > 1 && (
          <div className="field">
            <label>How each branch is represented</label>
            {branches.map((b) => (
              <div key={b.id} style={{ marginBottom: "0.5rem" }}>
                <span className="hint">{b.title} →</span>
                <input
                  aria-label={`How ${b.title} is represented`}
                  value={reps[b.id] ?? ""}
                  onChange={(e) => {
      ...
      {qualitiesCarried.length > 0 && (
        <div className="tag-row" aria-label="Qualities this action carries">
