...
export function AnglerHead({ x, y, scale = 1, color, loudness = 3, onClick }: Props) {
  ...
  return (
    <g
      className="branch-endpoint angler-head"
    ...
    >
      ...
      <circle
        ...
        pointerEvents="none"
      />
      <circle cx={4.2} cy={-3} r={0.9} fill={LURE_CORE} pointerEvents="none" />
      <path
        d={STALK}
        fill="none"
        stroke={color}
      ...
      <path d={BROW} fill={CAVE} opacity={0.55} stroke="none" pointerEvents="none" />

      {/* lower jaw drops with the gape, its needle teeth riding along */}
      <g transform={`translate(0 ${gape})`} pointerEvents="none">
      ...
      <circle cx={-3.6} cy={-3} r={0.75} fill={EYE} pointerEvents="none" />
      <circle cx={-3.5} cy={-3} r={0.38} fill={CAVE} pointerEvents="none" />

      <path d={FIN_SPINES} fill={color} opacity={0.8} stroke="none" pointerEvents="none" />
    </g>
  );
}
