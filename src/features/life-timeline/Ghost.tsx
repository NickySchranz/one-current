...
export function Ghost({ x, y, scale = 1, color, loudness = 3, onClick }: Props) {
  ...
  return (
    <g
      className="branch-endpoint ghost-wisp"
    ...
    >
      ...
      <path d={BODY} fill="#fff" opacity={0.45} stroke="none" pointerEvents="none" />

      {/* small reaching arms */}
      <path d={ARM_L} fill={color} opacity={0.75} stroke="none" pointerEvents="none" />
      <path d={ARM_R} fill={color} opacity={0.75} stroke="none" pointerEvents="none" />

      {/* hollow eyes, deepening as it gets louder */}
      <ellipse cx={-1.7} cy={-2.6} rx={0.85} ry={eyeRy} fill={HOLLOW} pointerEvents="none" />
      <ellipse cx={1.7} cy={-2.6} rx={0.85} ry={eyeRy} fill={HOLLOW} pointerEvents="none" />
